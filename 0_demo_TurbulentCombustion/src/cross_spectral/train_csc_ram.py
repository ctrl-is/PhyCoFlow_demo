...
from csc_ram import CSCRAMConfig, compute_csc_ram_cost
...
DEFAULTS = {
    ...
    "num_workers": 4,
    # Changed/Added here
    "save_dir": "Save_TrainedModel/csc_ram_tc_pointcloud",
    "graph_basis_path": "Save_Graph/graph_basis_k16_modes384",
    "csc_field_pairs": None,
    "csc_use_crossfreq_coupling": False,
    "csc_use_samefreq_coherence": False,
    "csc_use_samefreq_energy": True,
    "csc_use_crossfreq_energy": True,
    ...
    "t_eps": 1.0e-3,
    # Changed/Added here
    "reward_mode": "csc_band_energy",
    "csc_alpha_samefreq": 1.0,
    "csc_alpha_crossfreq": 1.0,
...
def _parse_field_pairs(value):
    ...
    for pair in value:
        if len(pair) != 2:
            raise ValueError(f"Pairs must be of length 2, got {pair}")
        pairs.append((int(pair[0]), int(pair[1])))
    
    return pairs

def build_csc_ram_config(cfg: dict) -> CSCRAMConfig:
     return CSCRAMConfig(
        eps=float(cfg.get("csc_eps", 1e-12)),
        alpha_samefreq=float(cfg.get("csc_alpha_samefreq", 1.0)),
        alpha_crossfreq=float(cfg.get("csc_alpha_crossfreq", 1.0)),
        field_pairs=_parse_field_pairs(cfg.get("csc_field_pairs", None)),
        use_samefreq_energy=bool(cfg.get("csc_use_samefreq_energy", True)),
        use_crossfreq_energy=bool(cfg.get("csc_use_crossfreq_energy", True)),
        use_samefreq_coherence=bool(cfg.get("csc_use_samefreq_coherence", False)),
        use_crossfreq_coupling=bool(cfg.get("csc_use_crossfreq_coupling", False)),
        use_denorm=bool(cfg.get("csc_use_denorm", False)),
    )
    ...
    *,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            cost, csc_metrics = compute_csc_ram_cost(
                fields_pred=x_end,
                fields_true=x_ref_g,
                U=graph_U,
                bands=graph_bands,
            ...
            rewards = -cost
            reward_point_count = x_end.shape[1]
...
def main():
    ...
    ram_coh_cfg = build_csc_ram_config(cfg)
